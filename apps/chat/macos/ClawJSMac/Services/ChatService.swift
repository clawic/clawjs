        ...
        Task {
            do {
                ...
                await MainActor.run {
                    ...
                    // Populate remote id maps with synthetic IDs so that local
                    // actions (createConversation, sendMessage) can proceed. The
                    // actual Relay calls will still fail, but ChatService
                    // already falls back to `fallbackReply` on error, so the UI
                    // stays responsive instead of being frozen.
                    for project in self.projects {
                        let remoteId = "mock-project-\(project.id.uuidString)"
                        self.projectMap[project.id] = remoteId
                        self.reverseProjectMap[remoteId] = project.id
                    }
                    for agent in self.agents {
                        let remoteId = "mock-agent-\(agent.id.uuidString)"
                        self.agentMap[agent.id] = remoteId
                        self.reverseAgentMap[remoteId] = agent.id
                    }
                    var pAgents: [UUID: [UUID]] = [:]
                    var aProjects: [UUID: [UUID]] = [:]
                    for project in self.projects {
                        pAgents[project.id] = self.agents.map { $0.id }
                    }
                    for agent in self.agents {
                        aProjects[agent.id] = self.projects.map { $0.id }
                    }
                    self.projectAgents = pAgents
                    self.agentProjects = aProjects
                }
            }
        }
    }
